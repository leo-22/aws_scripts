#!/bin/bash
#set -x

# Volume list file will have volume-id:Volume-name format

SNAPSHOT_NAMES=/temp/snapshot_names
VOLUMES_LIST=/temp/volumes-list
SNAPSHOT_INFO=/temp/snapshot_info
DATE=`date +%Y-%m-%d`
REGION="us-east-1"
OID="181218121805"

# Snapshots Retention Period for each volume snapshot
RETENTION=6

SNAP_CREATION=/temp/snap_creation
SNAP_DELETION=/temp/snap_deletion

EMAIL_LIST=leo.claus@sbsplanet.com

sudo echo "List of Snapshots Creation Status" > $SNAP_CREATION
sudo echo "List of Snapshots Deletion Status" > $SNAP_DELETION

# Check whether the volumes list file is available or not?

if [ -f $VOLUMES_LIST ]; then

# Creating Snapshot for each volume using for loop

for VOL_INFO in `cat $VOLUMES_LIST`
do
# Getting the Volume ID and Volume Name into the Separate Variables.

VOL_ID=`echo $VOL_INFO | awk -F":" '{print $1}'`
VOL_NAME=`echo $VOL_INFO | awk -F":" '{print $2}'`

# Creating the Snapshot of the Volumes with Proper Description.

DESCRIPTION="${VOL_NAME}_${DATE}"
###aws ec2 create-snapshot --volume-id $VOL_ID --description "$DESCRIPTION" --region $REGION & >> $SNAP_CREATION
done
else
echo "Volumes list file is not available : $VOLUMES_LIST Exiting." | mail -s "Snapshots Creation Status" $EMAIL_LIST
exit 1
fi

echo >> $SNAP_CREATION
echo >> $SNAP_CREATION

# Deleting the Snapshots which are 10 days old.

for VOL_INFO in `cat $VOLUMES_LIST`
do

   # Getting the Volume ID and Volume Name into the Separate Variables.

   VOLD_ID=`echo $VOL_INFO | awk -F":" '{print $1}'`
   VOL_NAME=`echo $VOL_INFO | awk -F":" '{print $2}'`

   # Getting the Snapshot details of each volume.

   aws ec2 describe-snapshots --query Snapshots[*].[SnapshotId,VolumeId,Description,StartTime] --output text --filter "Name=status,Values=completed" "Name=volume-id,Values=$VOL_ID" | grep -v "CreateImage" > $SNAPSHOT_INFO
done

# Create file with unique names of Snapshots in environment.
aws ec2 describe-snapshots --region $REGION --owner-id $OID |grep Value|sort -u|awk '{print $2}'|sed 's/"//g'|sed 's/,//g' > $SNAPSHOT_NAMES

# Snapshots Retention Period Checking and if it crosses delete them.

while read $SNAPSHOT_INFO
do
SNAP_ID=`echo $SNAPSHOT_INFO | awk '{print $1}'`
echo $SNAP_ID
SNAP_DATE=`echo $SNAPSHOT_INFO | awk '{print $4}' | awk -F"T" '{print $1}'`
echo `cat $SNAPSHOT_INFO --------`
echo $SNAP_DATE

# Getting the no.of days difference between a snapshot and present day.

RETENTION_DIFF=`echo $(($(($(date -d "$DATE" "+%s") - $(date -d "$SNAP_DATE" "+%s"))) / 86400))`
echo $RETENTION_DIFF

# Deleting the Snapshots which are older than the Retention Period

if [ $RETENTION -lt $RETENTION_DIFF ];
then
### aws ec2 delete-snapshot -snapshot-id $SNAP_ID --region $REGION --output text > /temp/snap_del
echo DELETING $SNAPSHOT_INFO >> $SNAP_DELETION
fi
done < $SNAPSHOT_INFO
done
echo >> $SNAP_DELETION

# Merging the Snap Creation and Deletion Data

sudo cat $SNAP_CREATION $SNAP_DELETION > /temp/mail_report

# Sending the mail Update

sudo cat /temp/mail_report | mail -s "Volume Snapshots Status" $EMAIL_LIST

##### End of script #####
